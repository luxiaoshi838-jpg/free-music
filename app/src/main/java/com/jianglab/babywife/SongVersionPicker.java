...
/**
 * Metadata-only manual song replacement picker.
 * Results and searched-source progress are retained per playlist song.
 */
final class SongVersionPicker {
    ...
    private SongVersionPicker(Activity activity, String identity, String title,
        ...
        this.identity = identity == null ? "" : identity.trim();
        ...
        String stableIdentity = this.identity.isEmpty()
    ...
    private void showDialog() {
        ...
        if (!rows.isEmpty()) {
            ...
            status.setText("已保留上次的 " + rows.size() + " 个替换结果"
    ...
    private void loadNext() {
        ...
        new Thread(() -> {
            ...
            activity.runOnUiThread(() -> {
                ...
                loading = false;
                if (error != null) {
                    status.setText("本次搜索异常，已保留现有结果；可下拉重试");
                    if (callback != null) callback.onStatus("搜索异常，自动任务已让路，可继续重试");
                    updateFooter();
                    return;
                }
        ...
        }, "manual-song-version-search").start();
    }
