...
# Confirm replacement: after all new metadata has been applied to target, explicitly
# synchronize the complete replacement metadata to the playlist before playback starts.
...
# After replacement cache resolves to the final real source, synchronize the final
# metadata again so the playlist shows exactly what the player is using.
old2 = '''                    persistResolvedCatalogToPlaylistCopies(song, originalKey);\n                    savePlaylists();\n                    renderCurrentPlaylist();'''
new2 = '''                    syncReplacementMetadataToPlaylistCopies(song, originalKey);\n                    renderCurrentPlaylist();'''
text = replace_once(text, old2, new2, "resolved replacement final sync")
